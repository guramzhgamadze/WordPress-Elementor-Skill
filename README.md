...
| **Elementor (free)** | 3.35.7+ (stable) | V3 `Widget_Base` API targets stable 3.35.7. **Elementor 4.0 Beta** launched March 16–19, 2026 (stable ~March 30). V3 widgets fully supported in 4.0; V4 Atomic extension API is Beta-only — all skill code uses V3. |
...
- **Round 17** — Audit pass (March 25, 2026): fixed missing `inert` attribute on off-canvas HTML panel (`offcanvas-ui.md` Bug #6 — keyboard focus gap before JS load); added JSON Schema draft-04 explanatory comment to `rest-api.md` (Bug #7); synced `SKILL.md` with Beta 6, RC1 delay, `WP_ALLOW_COLLABORATION` constant, Client-side Media Processing revert
- **Round 18** — Comprehensive re-audit (March 25, 2026): fixed backdrop overlay visible before JS load (`offcanvas-ui.md` — `myplugin-offcanvas__backdrop--hidden` class now set in HTML markup, not only by JS init); added pre-stable caveat to WP 7.0 AI Client API function names in `SKILL.md`; added source file citation for `woocommerce_hpos_enable_sync_on_read` filter in `woocommerce.md`; 46-file full scan — all other patterns verified clean
- **Round 19** — Full re-audit with live source checks (March 25, 2026): updated Elementor 4.0 status across `SKILL.md`, `elementor-patterns.md`, `js-css-standards.md`, and `README.md` — Elementor 4.0 Beta launched March 16–19, 2026 (stable ~March 30); V3 `Widget_Base` remains fully supported in 4.0; updated V4 rules preamble to reflect Atomic Editor is now the default for new 4.0 sites; all other patterns verified clean against official sources
